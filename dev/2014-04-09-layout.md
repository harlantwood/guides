...
which define the position and orientation and distortion of a node. While a
`Transform` is a static object, representing a snapshot in time, a `Modifier`
encapsulates varying `Transforms` over time, as well as bundling layout
properties like `size`, `origin` and `align`, `modifiers` can be directly added
to the Famo.us Render Tree, acting on the nodes beneath them.
...
A `Transform` combines these components into a 16-element array.
...
|`Transform.inFront`         |<pre>[1,      0,      0,      0, <br> 0,      1,      0,      0, <br> 0,      0,      1,      0, <br> 0,      0,      0.001,  1]</pre>|<pre> translation : [0,0, 0.001] <br> rotation : [0,0,0] <br> scale : [1,1,1] <br> skew : [0,θ,0] </pre>|A small z-translation forwards toward the viewer. Useful for layering.|
|`Transform.behind`          |<pre>[1,      0,      0,      0, <br> 0,      1,      0,      0, <br> 0,      0,      1,      0, <br> 0,      0,     -0.001,  1]</pre>|<pre> translation : [0,0,-0.001] <br> rotation : [0,0,0] <br> scale : [1,1,1] <br> skew : [0,θ,0] </pre>|A small z-translation backwards away from the viewer. Useful for layering.|
...
For this guide, we will be focused on the latter. `Transform` composition is done
through the `.multiply` method. Though `.multiply` is useful for composing
arbitrary transforms, it is heavyweight when all that is needed is to compound a
translation on a rotation. Famo.us provides several more performant methods for
...
|`Transform.thenMove(T, p)`| Translates a `transform` _T_ by `p = [x,y,z]`. <br> Equivalent to `Transform.multiply(Transform.translate(x,y,z), T)`.|
|`Transform.moveThen(p, T)`| Translates by `p = [x,y,z]` prior to applying the `Transform` _T_. <br> Equivalent to `Transform.multiply(T, Transform.translate(x,y,z))`.|
|`Transform.thenScale(T, s)`| Scales a `transform` _T_ by `s = [x,y,z]`. <br> Equivalent to `Transform.multiply(Transform.scale(x,y,z), T)`.|
|`Transform.aboutOrigin(T, p)`| Shifts the origin of a `Transform` to a new point `p = [x,y,z]` in pixels from the top/left. <br> For instance, <br> `Transform.aboutOrigin(Transform.rotateZ(Math.PI/4), [50,100,0])` <br> would rotate a renderable around a pivot at `[50,100,0]` instead of the top-left.|
...
|`Transform.interpret(T)`| Returns an object with `translate`, `rotate`, `scale`, and `skew` keys. <br> _Note_: Use is discouraged.|
...
and can be directly added to the Scene Graph. `Modifiers` modify the nodes (which
...
The following table summarizes common alignment values.
...
The `Modifier` class found in Famous/modifiers/StateModifier.js is a push-based
implementation. Here, `Modifier` has the methods

| method | description |
|--------|-------------|
|`Modifier.setTransform(T, definition, callback)`| Sets the transform state|
|`Modifier.setOpacity(opacity, definition, callack)`|Sets the opacity state|
|`Modifier.setSize(size, definition, callack)`|Sets the size state|
|`Modifier.setOrigin(origin, definition, callack)`|Sets the origin state|
|`Modifier.setAlign(align, definition, callack)`|Sets the align state|
